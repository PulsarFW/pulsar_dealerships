...
function OpenShowroomManagement(dealerId)
    if _dealerships[dealerId] and #_dealerships[dealerId].showroom > 0 then
        plsr.Callbacks:ServerCallback('Dealerships:ShowroomManagement:FetchData', dealerId, function(authed, stocks)
            local stockData = FormatDealerStockToCategories(stocks)
            
            if not authed then
                plsr.Notification:Error('You\'re Not Authorized to Open Showroom Management')
                return
            end

            showroomManagementSub = {}
            showroomManagement = plsr.Menu:Create('showroomManagement', string.format('Manage %s Showroom', _dealerships[dealerId].abbreviation), function()
...
function StartSettingShowroomPosition(stockData, dealerId, position, positionVehicle)
    showroomManagement = plsr.Menu:Create('showroomManagement', string.format('Setting %s Showroom Position #%s', _dealerships[dealerId].abbreviation, position), function()
    ...
    if positionVehicle then
        showroomManagement.Add:SubMenuBack('Clear Current Vehicle', {}, function()
            Wait(50)
            plsr.Callbacks:ServerCallback('Dealerships:ShowroomManagement:SetPosition', {
                dealerId = dealerId,
                position = position,
                vehData = false,
            }, function(success)
                if success then
                    plsr.Notification:Success('Successfully Cleared Vehicle in Position #'.. position)
                else
                    plsr.Notification:Error('Failed to Clear Vehicle in Position #'.. position)
                end
    ...
    for _, category in ipairs(orderedCategories) do
        if stockData.sorted[category] and #stockData.sorted[category] > 0 then
            showroomManagementSub[category] = plsr.Menu:Create('showroomManagementCat-'.. category, _catalogCategories[category])
...
function SetDealerShowroomVehicleAtPosition(dealerId, position, vehData)
    ...
    plsr.Game.Vehicles:SpawnLocal(vector3(playerCoords.x, playerCoords.y, playerCoords.z - 20.0), vehData.vehicle, 100, function(vehicle)
        FreezeEntityPosition(vehicle, true)
        local vehProperties = plsr.Vehicles.Properties:Get(vehicle)
        plsr.Game.Vehicles:Delete(vehicle)
        ...
        plsr.Callbacks:ServerCallback('Dealerships:ShowroomManagement:SetPosition', {
            ...
            if success then
                plsr.Notification:Success('Successfully Updated Vehicle in Position #'.. position)
            else
                plsr.Notification:Error('Failed to Update Vehicle in Position #'.. position)
            end
            OpenShowroomManagement(dealerId)
        end)
    end)
