...
function ViewDealershipStocks(dealerId)
    ...
    if dealerData then
        plsr.Callbacks:ServerCallback('Dealerships:StockViewing:FetchData', dealerId, function(authed, stocks, serverTime, dealerProfitPercent)
            if not authed then
                plsr.Notification:Error('You\'re Not Authorized to View Stock', 3500, 'car')
                return
            end
            ...
            stockViewing = plsr.Menu:Create('stockViewing', string.format('View %s Stock', dealerData.abbreviation), function()
            ...
            for _, category in ipairs(orderedCategories) do
                if stockData.sorted[category] and #stockData.sorted[category] > 0 then
                    stockViewingSub[category] = plsr.Menu:Create('stockViewingCat-'.. category, _catalogCategories[category])
                    ...
                    for k, v in ipairs(stockData.sorted[category]) do
                        stockViewingSub[category].Add:Text(string.format(
                            ...
                            v.price and '$' .. formatNumberToCurrency(math.floor(plsr.Utils:Round(v.price * saleMultiplier, 0))) or '$?',
