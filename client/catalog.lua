...
function GetVehicleDealerCarInformationText(vData)
	local profitPercentage = 1 + (_defaultDealershipSalesData.profitPercentage / 100)
	local vehiclePrice = string.format("%.0f", vData.price * profitPercentage)
	vehiclePrice = format_int(vehiclePrice)
	return (vData.make .. " " .. vData.model),
		string.format(
			[[
            Class: %s<br>
            Category: %s<br>
            Estimated Price: $%s<br>
        ]],
...
function OpenCatalogMenu(catalogName)
	...
	for _, cat in ipairs(orderedCategories) do
		if catalogData.sorted[cat] and #catalogData.sorted[cat] > 0 then
			...
			local vehicleInformationElement =
				catalogSubMenus[cat].Add:Text(informationData, { "pad", "center", "textLarge" })
